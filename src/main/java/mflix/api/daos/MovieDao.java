...
public class MovieDao extends AbstractMFlixDao {
  ...
  private boolean validIdValue(String movieId) {
    ...
    //Check out this method's use in the method that follows.
	  Bson query = eq("_id",movieId);
	  try {
		  
		  Document movie = moviesCollection.find(query).first();
		  if(movie != null) {
			  return true;
		  }else {
			  return false;
  ...
  public Document getMovie(String movieId) {
    ...
    Variable<String> reference = new Variable<String>("id", "$_id");
    List<Variable<String>> let = new ArrayList<>();
    let.add(reference);
    
    Bson lookupMatch = Aggregates.match(expr(eq("$eq",Arrays.asList("$movie_id","$$id"))));
    
    lookupPipeline.add(lookupMatch);
    lookupPipeline.add(Aggregates.sort(Sorts.descending("date")));
  ...
  public List<Document> getMoviesByCountry(String... country) {
	...
	//db.movies.find({countries: {$all:[ /countries list/ ]}},{title: 1})
